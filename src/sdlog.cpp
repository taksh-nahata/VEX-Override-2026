...
void log_loop() {
  ...
  fprintf(f,
          ...
          "lift_pos,lift_ma,lift_status\n");
  fflush(f);

  std::uint32_t start_ms = pros::millis();
  while (true) {
    const char* lift_status = lift::touched_down() ? "TOUCHED" : (lift::at_ceiling_now() ? "CEILING" : "");
    fprintf(f, "%u,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%s\n", pros::millis() - start_ms, chassis.drive_sensor_left(),
            chassis.drive_sensor_right(), chassis.imu.get_heading(), chassis.imu.get_pitch(), chassis.imu.get_roll(),
            lift::position(), lift::current_ma(), lift_status);
