...
- The screen-drawing library this PROS install actually compiled (LVGL 8.3.4, confirmed against PROS's own release notes) is an older version than the instructions/headers describe (v9). Not a mistake in our setup — this is genuinely how PROS ships kernel 4. First pass played it safe and fell back to the plain existing menu. Pushed further and found the real fix: pulled the actual correct version's source code directly from LVGL's own repository to get the exact right data format, checked every single function used against the compiled library one at a time, and deliberately left out custom button colors specifically because that one piece has a real size mismatch between the two versions — not worth risking for a color. Everything else (the logo, the buttons, the text) uses only the verified-safe parts.
