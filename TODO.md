...
- ~~Odometry tracking wheel offset~~ — **done 2026-09-26**: measured -1.97in with the LEFT-button calibration spin, now in `ODOM_HORIZONTAL_OFFSET`. Worth running that same button once more to double check — it now reports `odom_x`/`odom_y` too, which should stay near 0 after a spin if the fix actually took.
